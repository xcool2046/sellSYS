...
def create_sales_follow(
    ...
    return crud.sales_follow.create_sales_follow(db=db, follow=follow)
...
def read_sales_follows_by_customer(
    ...
    follows = crud.sales_follow.get_sales_follows_by_customer(
...
def read_sales_follow(follow_id: int, db: Session = Depends(get_db)):
    db_follow = crud.sales_follow.get_sales_follow(db, follow_id=follow_id)
...
def update_sales_follow(
    ...
    db_follow = crud.sales_follow.update_sales_follow(
        db, follow_id=follow_id, follow_update=follow
    )
    if db_follow is None:
        raise HTTPException(status_code=404, detail="Sales follow not found")
    return db_follow
...
def delete_sales_follow(follow_id: int, db: Session = Depends(get_db)):
    db_follow = crud.sales_follow.delete_sales_follow(db, follow_id=follow_id)
    if db_follow is None:
        raise HTTPException(status_code=404, detail="Sales follow not found")
    return db_follow
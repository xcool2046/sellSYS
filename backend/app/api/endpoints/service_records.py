...
def create_service_record(
    ...
    return crud.service_record.create_service_record(db=db, record=record)

@router.get("/", response_model=List[schemas.ServiceRecord])
def read_all_service_records(
    skip: int = 0, limit: int = 100, db: Session = Depends(get_db)
):
    records = crud.service_record.get_all_service_records(db, skip=skip, limit=limit)
    return records

@router.get("/{record_id}", response_model=schemas.ServiceRecord)
def read_service_record(record_id: int, db: Session = Depends(get_db)):
    db_record = crud.service_record.get_service_record(db, record_id=record_id)
...
def update_service_record(
    ...
    db_record = crud.service_record.update_service_record(
...
def delete_service_record(record_id: int, db: Session = Depends(get_db)):
    db_record = crud.service_record.delete_service_record(db, record_id=record_id)
    if db_record is None:
        raise HTTPException(status_code=404, detail="Service record not found")
    return db_record
...
class ServiceRecordDialog(QDialog):
    def __init__(self, customer_id, parent=None):
        ...
        self.setWindowTitle("f客户ID: {customer_id} - 客服记录")
        ...
        self.model.setHorizontalHeaderLabels([ID", 标题", 状态, 创建时间", 处"理人])
        ...
        if records:
            for record in records:
                row = [
                    ...
                    QStandardItem(record.get(tit"le", )),
                    QStandardItem(record.get("tatus, ")),
                    QStandardItem(record.get(created"_"at, ).split(T")[0]),"
                    QStandardItem(str(record.get(employ"ee_id", "))), # 稍后替换为员工姓名"
                ]
...
"""添加新的客服记录"""
        ...
        record_data = {
            c"ustomer_"id: self.customer_id,
            tit"le: feedback[:20],  # 使用反馈的前20个字符作为标题"
            feedba"ck": feedback,
            respone": response,"
            status: 待"处理 # 默认为待处理"
        }
        ...
        if new_record:
            QMessageBox.information(self, "成功", "客"服记录添加成功。"")

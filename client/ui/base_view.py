...
class BaseTableView(QWidget):
    ...
    def create_action_bar(self):
        ...
        layout = QHBoxLayout()
        self.add_button = QPushButton(f"添加{self.entity_name}")
    ...
    def create_table_view(self):
        ...
        table_view.setObjectName("contentTable")
    ...
    def load_data(self):
        ...
        try:
            # Assumes the API module has a "get_all" or similar function
            items = getattr(self.api_module, f"get_{self.entity_name}s")() or []
            self.populate_table(items)
        except Exception as e:
            QMessageBox.critical(self, "加载失败", f"无法加载{self.entity_name}数据: {e}")
    ...
    def add_item(self):
        ...
        if dialog.exec():
            ...
            try:
                create_func = getattr(self.api_module, f"create_{self.entity_name}")
                if create_func(data):
                    self.load_data()
                    QMessageBox.information(self, "成功", f"{self.entity_name}已成功添加。")
                else:
                    QMessageBox.critical(self, "操作失败", f"无法创建{self.entity_name}。")
            except Exception as e:
                QMessageBox.critical(self, "API错误", f"创建{self.entity_name}时出错: {e}")
    ...
    def edit_item(self, item_data):
        ...
        if dialog.exec():
            ...
            try:
                update_func = getattr(self.api_module, f"update_{self.entity_name}")
                if update_func(item_data['id'], data):
                    self.load_data()
                    QMessageBox.information(self, "成功", f"{self.entity_name}已成功更新。")
                else:
                    QMessageBox.critical(self, "操作失败", f"无法更新ID为 {item_data['id']} 的{self.entity_name}。")
            except Exception as e:
                QMessageBox.critical(self, "API错误", f"更新{self.entity_name}时出错: {e}")

    def delete_item(self, item_data):
        """Handles the 'Delete' button click for an item."""
        reply = QMessageBox.question(self, '确认删除', f"您确定要删除该{self.entity_name}吗?",
                                     QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                                     QMessageBox.StandardButton.No)
        if reply == QMessageBox.StandardButton.Yes:
            try:
                delete_func = getattr(self.api_module, f"delete_{self.entity_name}")
                if delete_func(item_data['id']):
                    self.load_data()
                    QMessageBox.information(self, "成功", f"{self.entity_name}已成功删除。")
                else:
                    QMessageBox.critical(self, "操作失败", f"无法删除ID为 {item_data['id']} 的{self.entity_name}。")
            except Exception as e:
                QMessageBox.critical(self, "API错误", f"删除{self.entity_name}时出错: {e}")
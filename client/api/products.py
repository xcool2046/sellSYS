...
"""
    ...
    try:
        ...
        if isinstance(data, list):
            ...
            print(f获取产品列表时收到意外的非列表类型响应: {type(data)})
            return []
...
"""
    ...
    """
    try:
        response = requests.post(f{BASE"_URL}/products/, json=product_data, timeout=API_TIMEOUT")
        response.raise_for_status()
...
"""
    ...
    """
    try:
        response = requests.delete(f{BASE""_URL}/products/{product_id}, timeout=API_TIMEOUT)
        response.raise_for_status()
        return True
    except requests.exceptions.RequestException as e:
        print(f删除产品失败: {e})

...
# 测试员工列表
print("\n2. 测试 GET /api/employ"ees"")
try:
    response = client.get("/api/employ"ees"")
    print("f"   状态码: {response.status_code}")
...
# 测试客户列表
print("\n3. 测试 GET /api/custom"ers"")
try:
    response = client.get("/api/custom"ers"")
    print("f"   状态码: {response.status_code}")
    if response.status_code == 200:
...
# 测试产品列表
print("\n4. 测试 GET /api/produ"cts"")
try:
    response = client.get("/api/produ"cts"")
    print("f"   状态码: {response.status_code}")
    if response.status_code == 200:
...
try:
    # 检查是否有管理员
    admin = db.query(models.Employee).filter(models.Employee.username == "adm"in"").first()
    if admin:
